<footer class="footer">
	<div class="container">
		<div class="footer__main">
			<div class="footer__col">
				<div class="footer__top-wrap">
						<a href="/" class="footer__logo" aria-label="На главную">
								<svg class="footer__logo-icon-1" width="124" height="104" viewBox="0 0 124 104">
		<use xlink:href="#icon-logo-1" />
	</svg>

								<svg class="footer__logo-icon-2" width="193" height="101" viewBox="0 0 193 101">
		<use xlink:href="#icon-logo-2" />
	</svg>

						</a>
					<ul class="footer__socials _mobile">
							<li>
								<a
									href="/"
									class="footer__social-link"
									aria-label="Однокласники"
									rel="nofollow noopener"
									target="_blank"
								>
										<svg  width="24" height="24" viewBox="0 0 24 24">
		<use xlink:href="#icon-ok" />
	</svg>

								</a>
							</li>
							<li>
								<a
									href="/"
									class="footer__social-link"
									aria-label="Вконтакте"
									rel="nofollow noopener"
									target="_blank"
								>
										<svg  width="24" height="24" viewBox="0 0 24 24">
		<use xlink:href="#icon-vk" />
	</svg>

								</a>
							</li>
					</ul>
				</div>
					<button class="button footer__btn _mobile" type="button">
			<svg class='button__icon' width="35" height="54" viewBox="0 0 35 54" fill="none" xmlns="http://www.w3.org/2000/svg">
				<g clip-path="url(#clip0_1407_48188)">
				<path fill-rule="evenodd" clip-rule="evenodd" d="M6.19325 19.7886C8.08162 16.6751 10.5217 12.6521 14.218 7.875C20.3115 0 30.4799 1.5301e-07 36.0783 1.4733e-07L181.46 0L277.922 2.1989e-07C283.52 2.1431e-07 293.689 0 299.782 7.875C303.478 12.6521 305.919 16.6751 307.806 19.7886C310.291 23.8847 311.82 26.4065 314 27C311.82 27.5935 310.291 30.1153 307.806 34.2114C305.919 37.3249 303.478 41.3479 299.782 46.125C293.689 54 283.52 54 277.922 54H180.96H36.0783C30.4799 54 20.3115 54 14.218 46.125C10.5217 41.3479 8.08162 37.3249 6.19325 34.2114C3.70889 30.1153 2.17937 27.5935 0 27C2.17937 26.4065 3.70888 23.8847 6.19325 19.7886Z" fill="currentColor"/>
				</g>
				<defs>
				<clipPath id="clip0_1407_48188">
				<rect width="35" height="54" fill="white"/>
				</clipPath>
				</defs>
			</svg>
		<span class="button__inner">
			<span class="button__text">ЗАДАТЬ ВОПРОС</span>
		</span>
			<svg class='button__icon _right' width="35" height="54" viewBox="0 0 35 54" fill="none" xmlns="http://www.w3.org/2000/svg">
				<g clip-path="url(#clip0_1407_48188)">
				<path fill-rule="evenodd" clip-rule="evenodd" d="M6.19325 19.7886C8.08162 16.6751 10.5217 12.6521 14.218 7.875C20.3115 0 30.4799 1.5301e-07 36.0783 1.4733e-07L181.46 0L277.922 2.1989e-07C283.52 2.1431e-07 293.689 0 299.782 7.875C303.478 12.6521 305.919 16.6751 307.806 19.7886C310.291 23.8847 311.82 26.4065 314 27C311.82 27.5935 310.291 30.1153 307.806 34.2114C305.919 37.3249 303.478 41.3479 299.782 46.125C293.689 54 283.52 54 277.922 54H180.96H36.0783C30.4799 54 20.3115 54 14.218 46.125C10.5217 41.3479 8.08162 37.3249 6.19325 34.2114C3.70889 30.1153 2.17937 27.5935 0 27C2.17937 26.4065 3.70888 23.8847 6.19325 19.7886Z" fill="currentColor"/>
				</g>
				<defs>
				<clipPath id="clip0_1407_48188">
				<rect width="35" height="54" fill="white"/>
				</clipPath>
				</defs>
			</svg>
	</button>

			</div>
			<div class="footer__col">
				<ul class="footer__contacts">
						<li>
								<p class="footer__contact-link">
										<svg class="footer__contact-icon" width="24" height="24" viewBox="0 0 24 24">
		<use xlink:href="#icon-pin" />
	</svg>

									<span>Адрес: ....</span>
								</p>
						</li>
						<li>
								<a href="#" class="footer__contact-link">
										<svg class="footer__contact-icon" width="24" height="24" viewBox="0 0 24 24">
		<use xlink:href="#icon-phone" />
	</svg>

									<span>+7 800 8877777</span>
								</a>
						</li>
						<li>
								<a href="#" class="footer__contact-link">
										<svg class="footer__contact-icon" width="24" height="24" viewBox="0 0 24 24">
		<use xlink:href="#icon-mail" />
	</svg>

									<span>mail@mail.ru</span>
								</a>
						</li>
				</ul>
				<ul class="footer__socials">
						<li>
							<a
								href="/"
								class="footer__social-link"
								aria-label="Однокласники"
								rel="nofollow noopener"
								target="_blank"
							>
									<svg  width="24" height="24" viewBox="0 0 24 24">
		<use xlink:href="#icon-ok" />
	</svg>

							</a>
						</li>
						<li>
							<a
								href="/"
								class="footer__social-link"
								aria-label="Вконтакте"
								rel="nofollow noopener"
								target="_blank"
							>
									<svg  width="24" height="24" viewBox="0 0 24 24">
		<use xlink:href="#icon-vk" />
	</svg>

							</a>
						</li>
				</ul>
			</div>
			<div class="footer__col">
				<p class="footer__statistics-title">Статистика посещений</p>
				<ul class="footer__statistics-list">
						<li>За последние сутки: 1620</li>
						<li>За последний месяц: 44823</li>
						<li>Последнее обновление: 26.12.2022</li>
				</ul>
			</div>
			<div class="footer__col">
				<ul class="footer__extras">
						<li>
							<a href="/" class="footer__extra-link ttu">ОБ ИСПОЛЬЗОВАНИИ ИНФОРМАЦИИ </a>
						</li>
						<li>
							<a href="/" class="footer__extra-link ttu">ОФИЦИАЛЬНЫЕ ВЕБ-РЕСУРСЫ</a>
						</li>
						<li>
							<a href="/" class="footer__extra-link ttu">НАПИСАТЬ АДМИНИСТРАТОРУ САЙТА</a>
						</li>
				</ul>
					<button class="button footer__btn" type="button">
			<svg class='button__icon' width="35" height="54" viewBox="0 0 35 54" fill="none" xmlns="http://www.w3.org/2000/svg">
				<g clip-path="url(#clip0_1407_48188)">
				<path fill-rule="evenodd" clip-rule="evenodd" d="M6.19325 19.7886C8.08162 16.6751 10.5217 12.6521 14.218 7.875C20.3115 0 30.4799 1.5301e-07 36.0783 1.4733e-07L181.46 0L277.922 2.1989e-07C283.52 2.1431e-07 293.689 0 299.782 7.875C303.478 12.6521 305.919 16.6751 307.806 19.7886C310.291 23.8847 311.82 26.4065 314 27C311.82 27.5935 310.291 30.1153 307.806 34.2114C305.919 37.3249 303.478 41.3479 299.782 46.125C293.689 54 283.52 54 277.922 54H180.96H36.0783C30.4799 54 20.3115 54 14.218 46.125C10.5217 41.3479 8.08162 37.3249 6.19325 34.2114C3.70889 30.1153 2.17937 27.5935 0 27C2.17937 26.4065 3.70888 23.8847 6.19325 19.7886Z" fill="currentColor"/>
				</g>
				<defs>
				<clipPath id="clip0_1407_48188">
				<rect width="35" height="54" fill="white"/>
				</clipPath>
				</defs>
			</svg>
		<span class="button__inner">
			<span class="button__text">ЗАДАТЬ ВОПРОС</span>
		</span>
			<svg class='button__icon _right' width="35" height="54" viewBox="0 0 35 54" fill="none" xmlns="http://www.w3.org/2000/svg">
				<g clip-path="url(#clip0_1407_48188)">
				<path fill-rule="evenodd" clip-rule="evenodd" d="M6.19325 19.7886C8.08162 16.6751 10.5217 12.6521 14.218 7.875C20.3115 0 30.4799 1.5301e-07 36.0783 1.4733e-07L181.46 0L277.922 2.1989e-07C283.52 2.1431e-07 293.689 0 299.782 7.875C303.478 12.6521 305.919 16.6751 307.806 19.7886C310.291 23.8847 311.82 26.4065 314 27C311.82 27.5935 310.291 30.1153 307.806 34.2114C305.919 37.3249 303.478 41.3479 299.782 46.125C293.689 54 283.52 54 277.922 54H180.96H36.0783C30.4799 54 20.3115 54 14.218 46.125C10.5217 41.3479 8.08162 37.3249 6.19325 34.2114C3.70889 30.1153 2.17937 27.5935 0 27C2.17937 26.4065 3.70888 23.8847 6.19325 19.7886Z" fill="currentColor"/>
				</g>
				<defs>
				<clipPath id="clip0_1407_48188">
				<rect width="35" height="54" fill="white"/>
				</clipPath>
				</defs>
			</svg>
	</button>

			</div>
		</div>
		<div class="footer__bottom">
			<p class="footer__copy">© 2021 Copyright</p>
		</div>
	</div>
</footer>
		</div>
			<script src="assets/js/bundle.js?hash"></script>
	</body>
</html>